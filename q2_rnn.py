...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        s_pad = max(max_length - len(sentence), 0)
        sent_pad = sentence + [zero_vector]*s_pad
        labels_pad = labels + [zero_label]*s_pad
        mask = [True]*len(sentence) + [False]*s_pad
        ret.append((sent_pad[:max_length], labels_pad[:max_length], mask[:max_length]))
        pass
        ### END YOUR CODE ###
    return ret
