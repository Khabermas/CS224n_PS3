...
def make_windowed_data(data, start, end, window_size = 1):
	...
	for i in range(len(sentence)):
	     windowed_data.append(([y for x in padded[i:(i+(2*window_size+1))]
			for y in x], labels[i]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
	...
	embeddings = tf.nn.embedding_lookup(embed, self.input_placeholder)
        embeddings = embeddings = tf.reshape(embeddings, [-1, 	self.config.n_window_features*self.config.embed_size])                                                
        ### END YOUR CODE
        return embeddings
